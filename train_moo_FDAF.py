...
import random
from utils.train_utils import train_single, train_baseline, train_moo_fdaf, testing_funnction, CapturePrint, PrintCapture
from models.swin_transformer import swin_tiny
from models.gcn import vig_ti_224_gelu
from models.my_net import ViT2GNN, ViT2GNN_FDAF
...
if __name__ == '__main__':
	...
	for i in range(1):
		...
		with torch.no_grad():
			...
			#print(fusion_net)
			fusion_net = ViT2GNN_FDAF(swinT_base=swin_base, vig_base=gcn_base)
		fusion_net = fusion_net.to(device)
		fusion_weight = 1
		swin_weight = 0.5
		vig_weight = 0.6
		weight_save_path = f'./result/esophagel/fusion_ablation/fdaf_moo_{fusion_weight}*fusion_loss+{swin_weight}*swint_loss+{vig_weight}*vig_loss.pth'
		log_file_path = f'./result/esophagel/fusion_ablation/fdaf_moo_{fusion_weight}*fusion_loss+{swin_weight}*swint_loss+{vig_weight}*vig_loss_LOG.txt'
		...
		with contextlib.redirect_stdout(print_capture_1) as f:
			train_moo_fdaf(number=i+1, net=fusion_net, train_loader=train_loader, val_loader=val_loader, test_loader=test_loader,
