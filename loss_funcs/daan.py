...
class DAANLoss(AdversarialLoss, LambdaSheduler):
    def __init__(self, num_class, gamma=1.0, max_iter=1000, **kwargs):
        ...
        for _ in range(num_class):
            self.local_classifiers.append(Discriminator().cuda(0))

        self.d_g, self.d_l = 0, 0
        self.dynamic_factor = 0.5

    def forward(self, source, target):
        source_logits = F.softmax(source, dim=1)
    ...
    def get_local_adversarial_result(self, x, logits, c, source=True, lamb=1.0):
        ...
        for c in range(self.num_class):
            logits_c = logits[:, c].reshape((logits.shape[0], 1)).cuda(0)  # (B, 1)
            features_c = x * logits_c
            domain_pred = self.local_classifiers[c](features_c).cuda(0)
            ...
            #domain_pred = torch.squeeze(domain_pred, 2)
