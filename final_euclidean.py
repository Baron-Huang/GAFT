...
import random
from utils.train_utils import train_single, train_baseline, train_moo_dis_euclidean, testing_funnction, CapturePrint, PrintCapture
...
if __name__ == '__main__':
	...
	for i in range(1):
		...
		vig_weight = 0.6
		dis_weight = 0.45
		weight_save_path = f'./result/adv/coral_aamff_moo_dis_loss{fusion_weight}*fusion_loss+{swin_weight}*swint_loss+{vig_weight}*vig_loss+{dis_weight}*dis_loss_{i + 1}.pth'
		log_file_path = f'./result/adv/coral_aamff_moo_dis_loss{fusion_weight}*fusion_loss+{swin_weight}*swint_loss+{vig_weight}*vig_loss_LOG+{dis_weight}*dis_loss_LOG_{i + 1}.txt'
		...
		with contextlib.redirect_stdout(print_capture_1) as f:
			train_moo_dis_euclidean(number=i+1, net=fusion_net, train_loader=train_loader, val_loader=val_loader, test_loader=test_loader,
