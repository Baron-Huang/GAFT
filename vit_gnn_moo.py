...
import random
from models.vit_gnn_utils import train_moo
...
from models.vit_gnn import ViT2GNN
...
if __name__ == '__main__':
	...
	for i in range(5):
		...
		with torch.no_grad():
			fusion_net = ViT2GNN(vit_base=vit_base, vig_base=gcn_base)
			#print('\n', '########################## Net_summary #########################')
		...
		vig_weight = 0.5
		weight_save_path = f'./result/esophagel/vit_gnn/moo_{fusion_weight}*fusion_loss+{vit_weight}*vit_loss+{vig_weight}*vig_loss_{i + 1}.pth'
		log_file_path = f'./result/esophagel/vit_gnn/moo_{fusion_weight}*fusion_loss+{vit_weight}*vit_loss+{vig_weight}*vig_loss_LOG_{i + 1}.txt'
		...
		with contextlib.redirect_stdout(print_capture_1) as f:
			train_moo(number=i+1, fusion_net=fusion_net, train_loader=train_loader, val_loader=val_loader, test_loader=test_loader,
