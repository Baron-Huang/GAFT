...
import random
from utils.train_utils import train_single, train_baseline_w, train_moo, testing_funnction, CapturePrint, PrintCapture
...
if __name__ == '__main__':
	...
	for i in range(20):
		...
		fusion_weight = 0.1 + 0.1 * i #1.4
		weight_save_path = f'./result/esophagel/{fusion_weight}*fusion_weight_cnn_lr_w.pth'
		log_file_path = f'./result/esophagel/{fusion_weight}*fusion_weight_cnn_lr_w.txt'
		...
		with contextlib.redirect_stdout(print_capture_1) as f:
			train_baseline_w(number=i+1, fusion_net=fusion_net, train_loader=train_loader, val_loader=val_loader, test_loader=test_loader,
							gpu_device=device, epoch=epochs, weight_path=weight_save_path,fusion_weight=fusion_weight)
