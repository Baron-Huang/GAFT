...
import random
from utils.train_utils import train_single, train_baseline, train_moo, testing_funnction, CapturePrint, PrintCapture
from models.swin_transformer import swin_tiny
from models.gcn import vig_ti_224_gelu
from models.my_net import ViT2GNN
...
if __name__ == '__main__':
	...
	for i in range(1):
		...
		fusion_net = ViT2GNN(swinT_base=swin_base, vig_base=gcn_base)
		...
		final_path = r'/data/QYB/ViT2GNN/result/esophagel/all/moo_1*fusion_loss+0.5*swint_loss+0.6*vig_loss_2.pth'
		...
		features_extraction_without_classes(test_model=fusion_net,test_model_name='train_moo',data_loader=test_loader,gpu_device=device)
		# fusion_weight = 1
		# swin_weight = 0.5
		# vig_weight = 0.5
		# weight_save_path = f'./result/esophagel/moo_and_aamff/stg_moo_{fusion_weight}*fusion_loss+{swin_weight}*swint_loss+{vig_weight}*vig_loss_{i + 1}.pth'
		# log_file_path = f'./result/esophagel/moo_and_aamff/stg_moo_{fusion_weight}*fusion_loss+{swin_weight}*swint_loss+{vig_weight}*vig_loss_LOG_{i + 1}.txt'
		#
		...
		# 	train_moo(number=i+1, net=fusion_net, train_loader=train_loader, val_loader=val_loader, test_loader=test_loader,
