...
from models.my_net import ViT2GNN, ViT2GNN_AAMFF
from picture_out.picture_2B import features_extraction_without_classes
...
if __name__ == '__main__':
	...
	test_loader = DataLoader(test_data, batch_size=710, shuffle=False, num_workers=num_workers)
	...
	for i in range(1):
		...
		with torch.no_grad():
			...
			#print(fusion_net)
			fusion_net = ViT2GNN_AAMFF(swinT_base=swin_base, vig_base=gcn_base)
		...
		final_path = r'/data/QYB/ViT2GNN/result/esophagel/all/aamff_moo_1*fusion_loss+0.5*swint_loss+0.6*vig_loss.pth'
		fusion_net.load_state_dict(torch.load(final_path, map_location='cuda:1'))
		...
		features_extraction_without_classes(test_model=fusion_net, test_model_name='train_moo_aamff', data_loader=test_loader,
										gpu_device=device)
		# fusion_weight = 1
		# swin_weight = 0.5
		# vig_weight = 0.5
		# weight_save_path = f'./result/esophagel/aamff_moo_{fusion_weight}*fusion_loss+{swin_weight}*swint_loss+{vig_weight}*vig_loss.pth'
		# log_file_path = f'./result/esophagel/aamff_moo_{fusion_weight}*fusion_loss+{swin_weight}*swint_loss+{vig_weight}*vig_loss_LOG.txt'
		#
		...
		# 	train_moo_aamff(number=i+1, net=fusion_net, train_loader=train_loader, val_loader=val_loader, test_loader=test_loader,
		# 			fusion_weight=fusion_weight, swin_weight=swin_weight, vig_weight=vig_weight,gpu_device=device, epoch=epochs, weight_path=weight_save_path)
