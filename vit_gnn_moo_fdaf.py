...
import random
from models.vit_gnn_utils import train_moo, train_moo_fdaf
from utils.train_utils import testing_funnction, CapturePrint, PrintCapture
from models.swin_transformer import swin_tiny
from models.gcn import vig_ti_224_gelu
from models.vit_gnn import ViT2GNN_FDAF
from models.vit_module import VisionTransformer
...
if __name__ == '__main__':
	...
	ViT_path = r'/data/QYB/SVMamba/weight/ViT/jx_vit_base_p16_224-80ecf9dd.pth'
	...
	for i in range(5):
		...
		vit_weight_file = torch.load(ViT_path, map_location='cuda:1')
		...
		nn.init.trunc_normal_(vit_base.head.weight, std=.02)
		###########################################GCN_Part################################################
		...
		with torch.no_grad():
			fusion_net = ViT2GNN_FDAF(vit_base=vit_base, vig_base=gcn_base)
			#print('\n', '########################## Net_summary #########################')
		...
		fusion_weight = 1
		vit_weight = 0.5 + 0.1 * i
		vig_weight = 0.5
		weight_save_path = f'./result/esophagel/vit_gnn/moo_fdaf_{fusion_weight}*fusion_loss+{vit_weight}*vit_loss+{vig_weight}*vig_loss_{i + 1}.pth'
		log_file_path = f'./result/esophagel/vit_gnn/moo_fdaf_{fusion_weight}*fusion_loss+{vit_weight}*vit_loss+{vig_weight}*vig_loss_LOG_{i + 1}.txt'
		...
		with contextlib.redirect_stdout(print_capture_1) as f:
			train_moo_fdaf(number=i+1, fusion_net=fusion_net, train_loader=train_loader, val_loader=val_loader, test_loader=test_loader,
							gpu_device=device, epoch=epochs, fusion_weight=fusion_weight,vit_weight=vit_weight, vig_weight=vig_weight,
