...
import random
from utils.train_utils import train_single, train_baseline, train_moo_dis_adv, testing_funnction, CapturePrint, PrintCapture
...
if __name__ == '__main__':
	...
	#final_path = r'/data/QYB/ViT2GNN/result/lar/s/aamff_moo_dis_loss0.8*fusion_loss+0.8*swint_loss+0.8*vig_loss+0.25*dis_loss_4.pth'
	final_path = r'/data/QYB/ViT2GNN/result/esophagel/all/aamff_moo_dis_loss1*fusion_loss+0.5*swint_loss+0.6*vig_loss+0.45*dis_loss.pth'
	...
	for i in range(1):
		...
		larynx_weight = torch.load(final_path)
		fusion_net.load_state_dict(larynx_weight, strict=True)
		...
		out_mode = 'five'
		Grade = 'III'  # I, II, III
		datasets = 'eso'  # lar, eso
		print('########################## Visually analyze #########################')
		path_name = data_root + r'/Test_patch/' + str(Grade) + ''
		...
		for img_name in img_name_list:
			for idx, i in enumerate(block_select):
				my_cam = ViG_CAM(model=fusion_net, out_model='five', transform=transform, i=i,
		...
		# vig_weight = 0.5
		# dis_weight = 0.6
		# weight_save_path = f'./result/adv/aamff_moo_dis_loss{fusion_weight}*fusion_loss+{swin_weight}*swint_loss+{vig_weight}*vig_loss+{dis_weight}*dis_loss.pth'
		# log_file_path = f'./result/adv/aamff_moo_dis_loss{fusion_weight}*fusion_loss+{swin_weight}*swint_loss+{vig_weight}*vig_loss_LOG+{dis_weight}*dis_loss.txt'
		#
		...
		# 	train_moo_dis_adv(number=i+1, net=fusion_net, train_loader=train_loader, val_loader=val_loader, test_loader=test_loader,
