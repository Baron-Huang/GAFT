...
import random
from utils.train_utils import train_single, train_baseline, train_moo_dis, testing_funnction_dis, CapturePrint, PrintCapture
from models.swin_transformer import swin_tiny
from models.gcn import vig_ti_224_gelu
from models.my_net import ViT2GNN, ViT2GNN_FDAF, ViT2GNN_FDAF_Dis
...
if __name__ == '__main__':
	...
	for i in range(5):
		...
		with torch.no_grad():
			...
			#print(fusion_net)
			fusion_net = ViT2GNN_FDAF_Dis(swinT_base=swin_base, vig_base=gcn_base)
		...
		nn.init.trunc_normal_(fusion_net.fusion_block_dense_1.weight, std=.02)
		nn.init.trunc_normal_(fusion_net.fusion_block_dense_2.weight, std=.02)
		nn.init.trunc_normal_(fusion_net.fusion_layer.weight, std=.02)
		fusion_weight = 0.7
		swin_weight = 0.5         #8967
		vmamba_weight = 0.1      #8134
		dis_weight = 1.0#0.2     #9008
		weight_save_path = f'./result/esophagel/moo_and_aamff/stg_fdaf_moo_dis_loss_{fusion_weight}*fusion_loss+{swin_weight}*swint_loss+{vmamba_weight}*vig_loss+{dis_weight}*dis_loss.pth'
		log_file_path = f'./result/esophagel/moo_and_aamff/stg_fdaf_moo_dis_loss_{fusion_weight}*fusion_loss+{swin_weight}*swint_loss+{vmamba_weight}*vig_loss+{dis_weight}*dis_loss_LOG.txt'
		...
		with contextlib.redirect_stdout(print_capture_1) as f:
			train_moo_dis(number=i+1, net=fusion_net, train_loader=train_loader, val_loader=val_loader, test_loader=test_loader,
					fusion_weight=fusion_weight, swin_weight=swin_weight, vmamba_weight=vmamba_weight,dis_weight=dis_weight,
                          gpu_device=device, epoch=epochs, weight_path=weight_save_path)
		...
		with contextlib.redirect_stdout(print_capture_2) as f:
			testing_funnction_dis(test_model=fusion_net, train_loader=train_loader, val_loader=val_loader, test_loader=test_loader,
							  gpu_device=device, out_mode='triplet')
