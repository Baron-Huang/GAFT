...
if __name__ == '__main__':
	...
	image_size = 224
	num_workers = 1
	data_root = r'./dataset/Larynx_datasets_ST_MSLNet_Random'
	...
	for i in range(10):
		...
		swin_weight = 0.5
		vmamba_weight = 0.3
		dis_weight = 0.05 + i * 0.05#0.2
		weight_save_path = f'./result/lar/fdaf_moo_dis_loss_{fusion_weight}*fusion_loss+{swin_weight}*swint_loss+{vmamba_weight}*vig_loss+{dis_weight}*dis_loss.pth'
		log_file_path = f'./result/lar/fdaf_moo_dis_loss_{fusion_weight}*fusion_loss+{swin_weight}*swint_loss+{vmamba_weight}*vig_loss+{dis_weight}*dis_loss_LOG.txt'
		...
		with contextlib.redirect_stdout(print_capture_1) as f:
			train_moo_dis(number=1, net=fusion_net, train_loader=train_loader, val_loader=val_loader, test_loader=test_loader,
